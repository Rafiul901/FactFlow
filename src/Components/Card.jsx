...
const Card = ({ user }) => {
  ...
  return (
    <div className="w-full h-auto min-h-[280px] flex flex-col rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1">
      {/* Card content container */}
      <div className="flex-1 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 p-4 sm:p-6 text-white flex flex-col">
        
        {/* Title - responsive text and height */}
        <h2 className="text-xl sm:text-2xl font-bold mb-3 leading-tight line-clamp-2 min-h-[3.5rem] sm:min-h-[4rem]">
          {title}
        </h2>
        ...
        <div className="flex justify-end pt-2">
          <Link to={`/posts/${_id}`}>
            <button className="flex items-center px-3 py-2 sm:px-4 sm:py-2 bg-white text-black bg-opacity-20 rounded-full backdrop-blur-sm border border-blue-300 border-opacity-30 hover:bg-opacity-30 transition-all duration-300 group text-sm sm:text-base">
